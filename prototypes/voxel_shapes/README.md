...
- `compare_*.jpg` and `round2_*.jpg` are the sheets. `cockpit_height.jpg` shows the lead candidates from about 4 m up.
...
Build: `g++ -O2 -std=c++17 shapes2.cpp -o shapes2 && ./shapes2 OUTDIR` (and the same for shapes.cpp). Then `comp.py` makes the round-one sheets.
