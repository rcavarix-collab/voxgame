...
- **Sun shadows.** An orthographic 2048² depth map from the sun (`ShadowLightViewProj`, sky.h) over ±48–112 blocks around the player (wider with render distance), with the centre snapped to whole texels so the shadow edges don't crawl as the player moves. The sun crosses the sky in 50 minutes, so the map is re-rendered only when it's stale — the sun moved ~¼°, the player moved a quarter of the covered area, or chunk meshes changed — and most frames just reuse it (the profiler counts re-renders). The world pixel shader tests each pixel against it with 4 taps of hardware PCF (comparison sampler, border = lit); a normal offset of just over one texel plus a slope-scaled raster bias keep surfaces from shadowing themselves. Shadowed and sun-facing-away surfaces drop to 60 % light at full sun, fading out as the sun sets.
